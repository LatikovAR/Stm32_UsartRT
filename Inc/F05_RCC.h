...
#define FLASH_PREFETCH_ON()    { FLASH->ACR |= FLASH_ACR_PRFTBE;  }   //Turn On Flash prefetch(12 bytes per read)
...
inline void RCC_GPIOClockPin(uint32_t PPin) { 
  RCC->AHBENR |= RCC_AHBENR_GPIOAEN << (PPin >> 26);
}
